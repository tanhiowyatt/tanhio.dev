...
function markdownToHTML(markdown) {
  ...
  html = html.replace(/!\[(.*?)\]\((.*?)\)/gim, (match, alt, src) => {
    ...
    if (!src.startsWith('http') && !src.startsWith('/')) {
      ...
      const isSubpath = typeof globalThis !== 'undefined' ? globalThis.location?.pathname?.startsWith('/blog') : false;
...
function getBlogLanguage() {
  ...
  const primary = typeof navigator !== 'undefined' ? navigator.language?.split('-')[0]?.toLowerCase() : null;
  if (primary && ['en', 'pl', 'ru'].includes(primary)) return primary;
...
function renderPostHeader(headerContainer, frontmatter) {
  if (!headerContainer || !frontmatter.title) return;
  const dateStr = frontmatter.date 
    ? new Date(frontmatter.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    : '';
  const dateHtml = dateStr ? `<p>${dateStr}</p>` : '';
...
async function loadBlogPost() {
  ...
  try {
    ...
    renderPostHeader(headerContainer, frontmatter);
...
function handleCopySuccess(btn, textSpan, iconEl) {
  ...
  document.querySelectorAll('.rss-copy-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      navigator.clipboard.writeText(absoluteUrl).then(() => {
        ...
        if (textSpan && iconEl) {
          handleCopySuccess(btn, textSpan, iconEl);
        }
      }).catch(err => {
