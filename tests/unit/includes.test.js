...
describe('Includes', () => {
  ...
  describe('initCookieConsent', () => {
    ...
    test('should load Vercel Analytics scripts when choice is accept', () => {
      document.head.innerHTML = '';
      mockStorage['cookie-consent-choice'] = 'accept';
      Includes.initCookieConsent();
      
      const insightScript = document.querySelector('script[src="/_vercel/insights/script.js"]');
      const speedScript = document.querySelector('script[src="/_vercel/speed-insights/script.js"]');
      
      expect(insightScript).not.toBeNull();
      expect(speedScript).not.toBeNull();
      expect(insightScript.defer).toBe(true);
      expect(speedScript.defer).toBe(true);
    });

    test('should load Vercel Analytics scripts on Accept click', () => {
      document.head.innerHTML = '';
      Includes.initCookieConsent();
    ...
    test('should not load Vercel Analytics scripts when choice is decline', () => {
      document.head.innerHTML = '';
      mockStorage['cookie-consent-choice'] = 'decline';
      Includes.initCookieConsent();
      
      const insightScript = document.querySelector('script[src="/_vercel/insights/script.js"]');
      const speedScript = document.querySelector('script[src="/_vercel/speed-insights/script.js"]');
      
      expect(insightScript).toBeNull();
      expect(speedScript).toBeNull();
    });
  });
});
